...
impl Png {
    ...
    pub(crate) fn from_file(p0: &PathBuf) -> Result<Png> {
        use std::fs::File;
        use std::io::Read;

        let mut file = File::open(p0)?;
    ...
    pub(crate) fn save_to_file(&self, p0: &PathBuf) -> std::io::Result<()> {
        use std::fs::File;
        use std::io::Write;

        let mut file = File::create(p0)?;
    ...
    pub(crate) fn remove_first_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        ...
        if let Some(pos) = self.chunks.iter().position(|c| c.chunk_type() == &chunk_type) {
            ...
            Err(anyhow::anyhow!("Chunk type '{}' not found", chunk_type))
        }
    }
    fn header(&self) -> &[u8; 8] {
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self, Self::Error> {
        ...
        if bytes.len() < 8 {
            return Err(anyhow::anyhow!("PNG data too short"));
        }
        if bytes[..8] != Self::STANDARD_HEADER {
        ...
        while pos < bytes.len() {
            if pos + 4 > bytes.len() {
                return Err(anyhow::anyhow!("Unexpected end of data while reading chunk length"));
            }
            let length = u32::from_be_bytes([bytes[pos], bytes[pos+1], bytes[pos+2], bytes[pos+3]]) as usize;
            if pos + 12 + length > bytes.len() {
                return Err(anyhow::anyhow!("Unexpected end of data while reading chunk"));
            }
            let chunk_bytes = &bytes[pos..pos + 12 + length];
...
impl std::fmt::Display for Png {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        writeln!(f, "PNG with {} chunk(s):", self.chunks.len())?;
        for (i, chunk) in self.chunks.iter().enumerate() {
            writeln!(f, "  Chunk {}: {}", i + 1, chunk)?;
        }
        Ok(())
    }
}
...
mod tests {
    ...
    #[test]
    pub fn test_empty_png_as_bytes() {
        let actual = Png::from_chunks(Vec::<Chunk>::new());
        let expected: Vec<u8> = Png::STANDARD_HEADER.to_vec();
